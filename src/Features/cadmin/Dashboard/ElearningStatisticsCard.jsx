...
const ElearningStatisticsCard = () => {
  ...
  // Define the metrics buttons and cards data
  const metrics = [
    {
      key: "revenue",
      title: "Revenue",
  ...
  // Process data for chart when monthlyData is available
  const getChartData = () => {
    if (!monthlyData) return { xAxisLabels: [], barValues: [], maxValue: 0 };

    const months = Object.keys(monthlyData);
    const xAxisLabels = months.map(month => month.slice(0, 3)); // Jan, Feb, etc.
